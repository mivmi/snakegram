...
from snakegram.filters import build_filter, run_filter, FilterExpr
...
async def test_filter_expr_type_check():
    class_check_expr = FilterExpr(Operation.TYPE_OF, int, int)
    assert await run_filter(class_check_expr, None) is True

    instance_check_expr = FilterExpr(Operation.TYPE_OF, 5, int)
